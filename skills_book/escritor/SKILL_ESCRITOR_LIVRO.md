...
FUNCAO escrever_cena(cena, genero, bible, contexto_anterior, foco_usuario, falhas_anteriores=[]):
    ...
    SE falhas_anteriores NAO vazia:
        ...
        saida_atual = LER(f"{worktree}/_saida_escritor.md")
        ...
        SALVAR(f"{worktree}/_saida_escritor.md", saida_atual)
    ...
    SALVAR(f"{worktree}/_saida_escritor.md", prosa)
...
Arquivo: `{worktree}/_metadados_cena.json`
...
```
capitulo_04/
  _saida_escritor.md          # SUA SAIDA PRINCIPAL
  _metadados_cena.json        # SEUS METADADOS
  _afirmacoes_para_validar.json  # Atomizador (voce nao mexe)
  _resultado_march.json       # Validador MARCH (voce nao mexe)
  _resultado_continuidade.json  # Validador Continuidade (voce nao mexe)
  _saida_editor.md            # Editor (se houver, voce nao mexe)
  _saida_final.md             # Orquestrador copia daqui (editor ou escritor)
  _log_prompt_checker.md      # Auditoria MARCH (voce nao mexe)
  rascunhos/                  # Seu material de trabalho (descartavel)
```
...
**Sua unica saida: `_saida_escritor.md` + `_metadados_cena.json`.**