...
DE utils.constantes IMPORTAR (
    ...
    CHAVE_MARCH_RESULTADOS,
    CHAVE_MARCH_STATUS_GERAL,
    CHAVE_MARCH_TAXA_CONFIRMADOS
)

FUNCAO validar_cena_march(caminho_cena, caminho_corpus):
    perguntas = LER(f"{caminho_cena}/{PERGUNTAS_VALIDADOR_ARQ}")
    ...
    PARA CADA pergunta EM perguntas:
        ...
        SE evidencia.confirma:
            resultados.ADICIONAR({"id": pergunta.id, "status": MARCH_CONFIRMADO, "evidencia": evidencia.trecho, "tipo": pergunta.tipo})
        SENAO SE evidencia.contradiz:
            resultados.ADICIONAR({"id": pergunta.id, "status": MARCH_CONTRADITO, "evidencia": evidencia.trecho, "tipo": pergunta.tipo})
        SENAO:
            resultados.ADICIONAR({"id": pergunta.id, "status": MARCH_NAO_ENCONTRADO, "evidencia": null, "tipo": pergunta.tipo})
    ...
    status_geral = VALIDACAO_APROVADO SE contraditos == 0 E taxa >= MARCH_TAXA_CONFIRMACAO_MINIMA E nao_encontrados <= total * MARCH_TAXA_NAO_ENCONTRADO_MAXIMA SENAO VALIDACAO_REPROVADO

    SALVAR(f"{caminho_cena}/{RESULTADO_MARCH_ARQ}", {
...
- `{caminho_cena}/{PERGUNTAS_VALIDADOR_ARQ}` (array de perguntas do atomizador)
- `caminho_corpus` (pasta `{PASTA_CORPUS}/` com todos arquivos .md)

---

# 5. Formato de Saida (OBRIGATORIO)

Arquivo: `{caminho_cena}/{RESULTADO_MARCH_ARQ}`
