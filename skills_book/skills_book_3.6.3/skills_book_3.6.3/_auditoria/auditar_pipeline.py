...
# --------------------------------------------------------------------------- #
# Bloco A0 — decomposicao verificada (pre-requisito, v3.6.3)
# --------------------------------------------------------------------------- #

def auditar_decomposicao(raiz: Path, cenas: list[Path], ach: list[Achado]) -> dict:
    """Fase 0: nenhuma cena pode existir sem decomposicao verificada e aprovada."""
    d = raiz / "execucao" / "decomposicao"
    f_an = d / "_decomposicao_ufi.json"
    f_vf = d / "_resultado_verificacao_decomposicao.json"
    an, vf = ler_json(f_an), ler_json(f_vf)

    if not f_an.is_file():
        ach.append(Achado("BLOQUEIO", "obra", "A0.decomposicao_ausente",
                          "decomposicao nao verificada — execute o pipeline de decomposicao antes "
                          "de escrever cenas (falta execucao/decomposicao/_decomposicao_ufi.json)"))
    if not f_vf.is_file():
        ach.append(Achado("BLOQUEIO", "obra", "A0.decomposicao_nao_verificada",
                          "decomposicao nao verificada — falta "
                          "execucao/decomposicao/_resultado_verificacao_decomposicao.json"))
    elif vf is None:
        ach.append(Achado("BLOQUEIO", "obra", "A0.verificacao_ilegivel",
                          "_resultado_verificacao_decomposicao.json ilegivel"))
    else:
        if vf.get("decisao") != "APROVADO":
            ach.append(Achado("BLOQUEIO", "obra", "A0.decomposicao_nao_aprovada",
                              f"decisao do verificador = {vf.get('decisao')!r}; nenhuma cena pode "
                              f"comecar sem APROVADO"))
        if vf.get("violacao_cegueira") is True:
            ach.append(Achado("BLOQUEIO", "obra", "A0.violacao_cegueira",
                              "o verificador viu a analise do Agente 1 antes de fazer a propria; "
                              "a validacao nao vale"))
        ind = vf.get("analise_independente") or {}
        listas = [ind.get(k) for k in ("eventos", "entidades", "tensoes", "blocos_instrucionais")]
        if not any(isinstance(x, list) and x for x in listas):
            ach.append(Achado("ALERTA", "obra", "A0.cegueira_nao_auditavel",
                              "o verificador nao gravou a analise independente POR EXTENSO (so "
                              "contagem): a cegueira vira alegacao, nao fato auditavel"))

    # auto-aprovacao do Agente 1
    if an is not None and an.get("status_verificacao") == "APROVADO":
        if vf is None or vf.get("decisao") != "APROVADO":
            ach.append(Achado("BLOQUEIO", "obra", "A0.auto_aprovacao",
                              "_decomposicao_ufi.json declara status_verificacao=APROVADO sem "
                              "aprovacao correspondente do verificador: o Agente 1 aprovou a si mesmo"))

    # plano derivou da decomposicao aprovada?
    if an is not None and isinstance(an.get("total_cenas_proposto"), int) and cenas:
        prop = an["total_cenas_proposto"]
        if len(cenas) > prop:
            ach.append(Achado("ALERTA", "obra", "A0.plano_divergente",
                              f"{len(cenas)} cena(s) em producao, mas a decomposicao aprovada propoe "
                              f"{prop}: o plano derivou da decomposicao validada"))

    return {"decomposicao_presente": f_an.is_file(), "verificacao_presente": f_vf.is_file(),
            "decisao": (vf or {}).get("decisao"),
            "total_cenas_proposto": (an or {}).get("total_cenas_proposto")}
...
def render_md(rep: dict) -> str:
    ...
    L += ["| Severidade | Qtd |", "|---|---:|",
          ...
          f"| INFO | {c['INFO']} |", ""]
    d = rep.get("decomposicao", {})
    L += ["## Fase 0 — decomposição verificada?", "",
          f"- Decomposição presente: {'sim' if d.get('decomposicao_presente') else 'NÃO'}",
          f"- Verificação presente: {'sim' if d.get('verificacao_presente') else 'NÃO'}",
          f"- Decisão do verificador: **{d.get('decisao') or 'ausente'}**",
          f"- Cenas propostas na decomposição: {d.get('total_cenas_proposto', 'n/d')}", ""]
...
def main() -> int:
    ...
    decomp = auditar_decomposicao(raiz, cenas, achados)
