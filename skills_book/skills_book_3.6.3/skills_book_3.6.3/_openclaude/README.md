# Adaptador OpenClaude — Skills Book v3.6.3


12 papéis convertidos de "roteiro lido na mesma janela" para **subagentes com contexto isolado de verdade**.


| Agente | Papel original | Cérebro |
|---|---|---|
| `book-orquestrador` | Orquestrador | `cerebros/orquestrador.md` |
| `book-analista-de-decomposicao` | Analista de Decomposição | `cerebros/analista-de-decomposicao.md` |
| `book-verificador-de-decomposicao` | Verificador de Decomposição | `cerebros/verificador-de-decomposicao.md` |
...
> **Já vem instalado.** `.openclaude/agents/` existe na raiz deste sistema com os
> 12 adaptadores prontos. Rode o instalador só para **reinstalar**,
> **verificar** ou depois de mexer nos arquivos.

```bash
cd "<...>/skills_book_3.6.3"
python3 _openclaude/instalar.py             # instala e verifica
python3 _openclaude/instalar.py --verificar # só confere, não copia
```
...
644), então `./_openclaude/instalar.sh` falha com `Permission denied` na sua máquina. E no
...
(`model` não vai no frontmatter, vai em `settings.json`),
...
Depois, rode o OpenClaude **com o diretório de trabalho na raiz do sistema** — os
...
```json
{
  ...
  "agentRouting": {
    "book-orquestrador": "barato",
    "book-analista-de-decomposicao": "barato",
...
Todos os 12 papéis deste sistema se comunicam **de forma hierárquica** —
