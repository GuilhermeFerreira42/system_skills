...
FUNCAO consolidar_livro(plano, estado, output_path):
    ...
    // filtro: so status == "CONCLUIDO"

    partes = []
    
    PARA CADA cena EM cenas_ordenadas:
        worktree = f"capitulos/capitulo_{cena.capitulo:02d}/"
        
        // Ler saida final (editor se houver, senao escritor)
        SE ARQUIVO_EXISTE(f"{worktree}/_saida_editor.md"):
            prosa = LER(f"{worktree}/_saida_editor.md")
        SENAO:
            prosa = LER(f"{worktree}/_saida_escritor.md")
        
        metadados = LER(f"{worktree}/_metadados_cena.json")
...
def validar_fronteira(livro_final, plano, estado):
    ...
    PARA CADA cena EM cenas_incluidas:
        prosa_original = LER(f"capitulos/capitulo_{cena.cap:02d}/_saida_final.md")
